...
if "%choice%"=="1" (
    ...
    docker run --gpus all --rm ai-data-creator-inpainting:stable python test_dependencies.py
) else if "%choice%"=="2" (
    echo Running quick inpainting test...
    docker run --gpus all --rm ^
        -v "%cd%\data:/app/data" ^
        ai-data-creator-inpainting:stable python test_inpainting.py
) else if "%choice%"=="3" (
    ...
    docker run --gpus all -it --rm ^
        ...
        ai-data-creator-inpainting:stable
) else if "%choice%"=="4" (
...
if not "%choice%"=="5" (
    ...
    docker images | findstr ai-data-creator-inpainting
    echo.
