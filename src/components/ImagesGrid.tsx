...
export function ImagesGrid({
  ...
  // Create a map of image IDs to their load states
  const imageLoadStates = Object.fromEntries(
    images.map(image => [
      image.id,
      useImageLoad(image.thumbnailUrl)
    ])
  );
      ...
      {!images.length ? (
        ...
        <AnimatePresence mode="wait">
          {images.map((image) => {
            ...
            const { isLoaded, getImageFadeProps } = imageLoadStates[image.id];
